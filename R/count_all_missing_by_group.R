...
#'
#' @description Given a data frame `data` and a column `group`, `count_all_missing_by_group()`
#'    creates a new data frame with one row per level of `group`. The first column of the output
#'    data frame contains the levels of `group`, and the rest of the columns contain the number
#'    of missing values for all columns in `data` except `group`.
#'
#' @details This function is a wrapper around the `group_by()` and `summarize()` functions
...
#'    drops all levels of grouping (`"drop"`). Specify `"keep"` to keep the output data frame
#'    grouped by `group`. See `summarize()` documentation in `dplyr` for a full list of options.
#'
